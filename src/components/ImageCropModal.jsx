      ...
      <div className="bg-engine-panel border border-engine-panel-border/30 p-6 rounded-2xl shadow-panel-neon max-w-lg w-full">
        ...
        {file.type === 'image/gif' ? (
          <div className="text-center py-8">
            ...
            <img src={imageSrc} alt="Preview" className="w-32 h-32 rounded-full object-cover mx-auto border-2 border-engine-panel-border/50 shadow-panel-neon" />
          </div>
        ) : (
          <>
        ...
        <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-engine-panel-border/10">
          ...
          <button
            onClick={handleApply}
            disabled={isProcessing}
            className="bg-engine-button text-engine-bg px-6 py-2 rounded-xl text-sm font-extrabold hover:bg-[#00e5ff] hover:shadow-button-neon transition-all flex items-center gap-2 disabled:opacity-50"
          >
