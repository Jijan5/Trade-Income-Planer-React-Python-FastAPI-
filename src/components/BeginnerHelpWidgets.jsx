    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-engine flex items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto">
      <div className="bg-engine-panel rounded-2xl border border-engine-panel-border/30 shadow-panel-neon w-full max-w-[95vw] sm:max-w-lg md:max-w-xl lg:max-w-2xl my-4 max-h-[90vh] flex flex-col relative overflow-hidden">
        ...
        <div className="p-4 sm:p-5 border-b border-engine-panel-border/20 flex justify-between items-center bg-engine-bg/80 shrink-0">
          ...
          <button
            onClick={onClose}
            className="text-engine-neon/50 hover:text-engine-neon font-bold p-1.5 transition-colors drop-shadow-panel-neon hover:drop-shadow-panel-neon"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-5 flex-1 overflow-hidden flex flex-col min-h-0 bg-engine-panel">
          <input
            ...
            className="w-full bg-engine-bg border border-engine-panel-border/30 rounded-xl text-white p-3 sm:p-4 mb-4 focus:border-engine-panel-border focus:shadow-panel-neon outline-none text-sm font-mono uppercase tracking-widest transition-all placeholder-[#00cfff]/30"
          />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 overflow-y-auto flex-1 min-h-0 pr-2 custom-scrollbar">
            {filteredTerms.map(([key, value]) => (
              <button
                ...
                className="text-left p-3 bg-engine-bg/60 hover:bg-engine-button/10 rounded-xl border border-engine-button-border/20 hover:border-engine-button-border/50 transition-all hover:shadow-button-neon hover:-translate-y-0.5"
              >
                <span className="text-xs sm:text-sm font-extrabold text-engine-neon uppercase tracking-wider drop-shadow-panel-neon">
                  {value.term}
                </span>
              </button>
            ))}
        ...
        {selectedTerm && (
          <div className="p-4 sm:p-5 border-t border-engine-panel-border/20 bg-engine-bg/90 shrink-0 animate-fade-in relative">
            <h3 className="text-lg sm:text-xl font-extrabold text-engine-neon mb-3 uppercase tracking-widest drop-shadow-panel-neon">
              {selectedTerm.term}
            </h3>
            <p className="text-gray-300 text-sm mb-4 leading-relaxed font-medium">
              {selectedTerm.definition}
            </p>
            <div className="bg-engine-button/5 p-4 rounded-xl border border-engine-button-border/20 shadow-[inset_0_0_10px_rgba(var(--engine-neon-rgb),0.05)]">
              <span className="text-[10px] text-engine-neon uppercase font-extrabold tracking-widest block mb-2 border-b border-engine-panel-border/20 pb-1">
                Example
              </span>
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-engine flex items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto">
      <div className="bg-engine-panel rounded-2xl border border-engine-panel-border/30 shadow-panel-neon w-full max-w-[95vw] sm:max-w-md my-4 max-h-[90vh] flex flex-col relative overflow-hidden">
            ...
            <label
              ...
              className={`flex items-start gap-3 p-3 sm:p-4 rounded-xl border cursor-pointer transition-all ${
                checkedItems[item.id]
                  ...
                  : "bg-engine-bg/50 border-engine-panel-border/20 hover:border-engine-panel-border/40"
              }`}
            >
              <div className="relative flex items-start">
                <input
                  ...
                  className="mt-0.5 sm:mt-1 w-5 h-5 rounded border-engine-panel-border/50 bg-engine-bg accent-green-500 focus:ring-green-500 focus:ring-offset-0 cursor-pointer appearance-none checked:bg-green-500 checked:border-green-500 transition-colors"
                />
                {checkedItems[item.id] && (
        ...
        <div className="p-4 sm:p-5 border-t border-engine-panel-border/20 bg-engine-bg/90 shrink-0">
          {showConfirmed ? (
            ...
            <button
              ...
              className={`w-full py-3 sm:py-4 rounded-xl font-extrabold uppercase tracking-widest transition-all text-sm sm:text-base ${
                allRequiredChecked
                  ...
                  : "bg-gray-800 text-gray-500 border border-engine-panel-border/30 cursor-not-allowed"
              }`}
            >
              {allRequiredChecked
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-engine flex items-center justify-center z-50 p-4 sm:p-6 overflow-y-auto">
      <div className="bg-engine-panel rounded-2xl border border-engine-panel-border/40 w-full max-w-[95vw] sm:max-w-lg my-4 shadow-panel-neon overflow-hidden relative">
        ...
        <div className="p-6 sm:p-10 text-center relative z-10">
          <div className="text-5xl sm:text-7xl mb-6 sm:mb-8 drop-shadow-panel-neon animate-fade-in" key={currentStep}>
            {step.icon}
          </div>
          ...
          <div className="flex justify-between items-center gap-3">
            ...
            <button
              onClick={handleNext}
              className="bg-engine-button hover:bg-[#00e5ff] text-engine-bg px-6 sm:px-8 py-3 rounded-xl font-extrabold uppercase tracking-widest transition-all shadow-button-neon hover:shadow-button-neon hover:-translate-y-0.5 text-sm sm:text-base"
            >
          ...
          <label className="flex items-center justify-center gap-2 mt-6 sm:mt-8 cursor-pointer group">
            <div className="relative flex items-center justify-center">
              <input
                ...
                className="w-4 h-4 rounded border-engine-button-border/50 bg-engine-bg appearance-none cursor-pointer checked:bg-engine-button checked:border-engine-button-border transition-colors"
              />
              {dontShowAgain && (
