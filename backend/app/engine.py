...
def analyze_trade_health(request):
  ...
  if not trades:
    return HealthAnalysisResponse(
      ...
      summary="Not enough data.", warning=[], recommendations_risk=1.0,
  ...
  # 2. risk analysis
  # check if any single loss > 3% of balance (aggressive/reckless)
  high_risk_trades = [t for t in losses if abs(float(t.pnl)) > float(t.balance) * 0.03]
  
  #check consistency of risk amount (standar deviation)
  ...
  risk_score = 100
  if high_risk_trades: risk_score -= 30
  if avg_risk > 0 and risk_std_dev > avg_risk * 0.5: risk_score -= 20
  ...
  for i in range (len(trades)):
    if not trades[i].is_win:
      ...
      # check if next trade risk increased significantly
      if i + 1 < len(trades):
