...
def send_contact_reply_email(recipient_email: str, subject: str, message: str):
    ...
    if IS_PRODUCTION and SENDGRID_API_KEY:
        ...
        try:
            sg = SendGridAPIClient(SENDGRID_API_KEY)
            response = sg.send(sg_message)
            print(f"SendGrid reply email sent successfully! Status code: {response.status_code}")
        except Exception as e:
            print(f"Failed to send reply email via SendGrid: {e}")
            raise e
    else:
...
def send_password_reset_email(recipient_email: str, pin: str):
    ...
    if IS_PRODUCTION and SENDGRID_API_KEY:
        sg_message = Mail(
            from_email=SMTP_USER,
            to_emails=recipient_email,
            subject="Password Reset PIN - Trade Income Planner",
            html_content=body
        )
        try:
            sg = SendGridAPIClient(SENDGRID_API_KEY)
            response = sg.send(sg_message)
            print(f"SendGrid password reset email sent successfully! Status code: {response.status_code}")
            return True
        except Exception as e:
            print(f"Failed to send password reset email via SendGrid: {e}")
            return False
    else:
        if not all([SMTP_USER, SMTP_PASSWORD, SMTP_SERVER]):
